...
public typealias CalendarEventHelperCompletion = (_ error: Error?) -> Void

/// A closure executed when trying to get an event from calendar.
public typealias CalendarEventHelperEventCompletion = (_ event: EKEvent?, _ error: Error?) -> Void

/// A closure executed when trying to get a bunch of events from calendar.
public typealias CalendarEventHelperEventsCompletion = (_ events: [EKEvent]?, _ error: Error?) -> Void

/// A closure executed when requesting user authorization to the calendar.
public typealias CalendarEventHelperAuthorizationCompletion = (_ granted: Bool, _ status: EKAuthorizationStatus, _ error: Error?) -> Void


public class CalendarEventHelper {
    
    /// A singleton object which mainly stored the default `eventStore`.
    fileprivate static let manager = CalendarEventHelper()
    ...
    public class func requestAuthorization(_ completion: CalendarEventHelperAuthorizationCompletion? = nil) {
        let status = EKEventStore.authorizationStatus(for: .event)
        switch status {
        case .authorized:
            completion?(true, status, nil)
        case .notDetermined:
            CalendarEventHelper.manager.eventStore
                .requestAccess(to: .event) { (granted, error) in
                    completion?(granted, status, error)
            }
        case .restricted:
            completion?(false, status, CalendarEventHelperError.authorizationFailed(reason: .authorizationRestricted))
        case .denied:
            completion?(false, status, CalendarEventHelperError.authorizationFailed(reason: .authorizationDenied))
        }
    }
    ...
    public class func getEventStore() -> EKEventStore {
        return CalendarEventHelper.manager.eventStore
    }
    ...
    public class func newEvent(in calendar: EKCalendar? = nil) -> EKEvent {
        let eventStore = CalendarEventHelper.manager.eventStore
    ...
    public class func getEvent(with identifier: String, completion: @escaping CalendarEventHelperEventCompletion) {
        execute({
            let eventStore = CalendarEventHelper.manager.eventStore
    ...
    public class func getEvents(from date: Date, to: Date, in calendars: [EKCalendar]? = nil, completion: @escaping CalendarEventHelperEventsCompletion) {
        execute({
            let eventStore = CalendarEventHelper.manager.eventStore
    ...
    public class func deleteEvent(with identifier: String, span: EKSpan = .thisEvent, completion: CalendarEventHelperCompletion? = nil) {
        execute({
    ...
    public class func resetStore() {
        CalendarEventHelper.manager.eventStore.reset()
    }
    ...
    public class func commitChanges(_ completion: CalendarEventHelperCompletion? = nil) {
        executeAndThrow({
            try CalendarEventHelper.manager.eventStore.commit()
    ...
    public class func newCalendar(in eventStore: EKEventStore? = nil, source: EKSource? = nil) -> EKCalendar {
        if let eventStore = eventStore {
            CalendarEventHelper.manager.eventStore = eventStore
        }
        
        let calendar = EKCalendar(for: .event, eventStore: CalendarEventHelper.manager.eventStore)
        calendar.source = source ?? CalendarEventHelper.manager.eventStore.defaultCalendarForNewEvents?.source
        return calendar
    }
    ...
    public class func getCalendars() -> [EKCalendar] {
        return CalendarEventHelper.manager.eventStore.calendars(for: .event)
    }
    
    
    // MARK: - Private
    fileprivate class func execute(_ block: @escaping (() -> ()), completion: CalendarEventHelperCompletion? = nil) {
    ...
    fileprivate class func executeAndThrow(_ block: @escaping (() throws -> ()), completion: CalendarEventHelperCompletion? = nil) {
        requestAuthorization { (granted, _, error) in
            if granted {
                do {
                    try block()
...
public extension EKEvent {
    ...
    public func save(span: EKSpan = .thisEvent, completion: CalendarEventHelperCompletion? = nil) {
        CalendarEventHelper.executeAndThrow({
            try CalendarEventHelper.manager.eventStore.save(self, span: span)
            DispatchQueue.main.async { completion?(nil) }
        }, completion: completion)
    }
    ...
    /// - parameter completion:     The `completion` closure to be called after the event deletion.
    public func delete(span: EKSpan = .thisEvent, commit: Bool = true, completion: CalendarEventHelperCompletion? = nil) {
        CalendarEventHelper.executeAndThrow({
            try CalendarEventHelper.manager.eventStore.remove(self, span: span, commit: commit)
            DispatchQueue.main.async { completion?(nil) }
        }, completion: completion)
    }
}
...
public extension EKCalendar {
    ...
    public func save(commit: Bool = true, completion: CalendarEventHelperCompletion? = nil) {
        CalendarEventHelper.executeAndThrow({
            try CalendarEventHelper.manager.eventStore.saveCalendar(self, commit: commit)
            DispatchQueue.main.async { completion?(nil) }
        }, completion: completion)
    }
    ...
    public func delete(commit: Bool = true, completion: CalendarEventHelperCompletion? = nil) {
        CalendarEventHelper.executeAndThrow({
            try CalendarEventHelper.manager.eventStore.removeCalendar(self, commit: commit)
            DispatchQueue.main.async { completion?(nil) }
        }, completion: completion)
    }
}
